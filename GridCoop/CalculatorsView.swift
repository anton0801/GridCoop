...
final class WebCoordinator: NSObject {
    ...
    private let cookieJar = CoopConstants.cookieNest
    
    func loadURL(_ url: URL, in webView: WKWebView) {
        print("\(CoopConstants.logHay) Load: \(url.absoluteString)")
        redirectCount = 0
...
extension WebCoordinator: WKNavigationDelegate {
    ...
    func webView(_ webView: WKWebView, didCommit navigation: WKNavigation!) {
        if let current = webView.url { checkpoint = current; print("✅ \(CoopConstants.logHay) Commit: \(current.absoluteString)") }
    }
